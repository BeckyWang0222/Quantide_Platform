...
REDIS_DB = 0
REDIS_QUEUE_JSON = 'benchmark:json'
REDIS_QUEUE_CSV = 'benchmark:csv'

class FormatBenchmark:
    """JSON和CSV格式性能对比基准测试（使用pandas生成数据）"""

    def __init__(self):
        """初始化基准测试"""
        # 设置字段顺序，按照要求的数据结构
        self.field_order = ['symbol', 'frame', 'open', 'high', 'low', 'close', 'vol', 'amount']
    ...
    def serialize_csv(self, df: pd.DataFrame) -> str:
        ...
        metadata = pd.DataFrame([{
            self.field_order[0]: 0,  # symbol为整数
            self.field_order[1]: datetime.now().date(),  # frame为日期
            self.field_order[2]: len(df)  # 使用open字段存储记录数
        }])

        # 合并元数据和数据
        combined_df = pd.concat([metadata, df], ignore_index=True)

        # 转换为CSV字符串（不包含索引和列名）
        output = io.StringIO()
        combined_df.to_csv(output, index=False, header=False)
        return output.getvalue()

    def deserialize_csv(self, csv_str: str) -> pd.DataFrame:
        """CSV反序列化（使用pandas，无key）
    ...
    def benchmark_redis_operations(self, df: pd.DataFrame, iterations: int = 5) -> Dict[str, float]:
        ...
        for _ in range(iterations):
            ...
            for batch_df in dfs:
                ...
                output = io.StringIO()
                # 写入元数据行（符合新的数据结构）
                output.write(f"0,{datetime.now().date()},{len(batch_df)},0,0,0,0,0,0\n")

                # 写入数据行（不带列名）
                batch_df.to_csv(output, mode='a', index=False, header=False)
    ...
    def _plot_results(self, results):
        ...
        # 保存为CSV
        df.to_csv('results/pandas_benchmark.csv', index=False)
        ...
        plt.tight_layout()
        plt.savefig('results/pandas_serialization_benchmark.png')
        ...
        plt.tight_layout()
        plt.savefig('results/pandas_size_benchmark.png')
        ...
        plt.tight_layout()
        plt.savefig('results/pandas_redis_benchmark.png')
