...
class DataProcessor:
    ...
    def _ensure_data_types(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col, dtype in type_dict.items():
            if col in data.columns:
                try:
                    if col == 'frame' and data[col].dtype != 'datetime64[ns]':
                        data[col] = pd.to_datetime(data[col])
                    elif col == 'trade_date' and 'frame' not in data.columns:
                        # 如果有trade_date列但没有frame列，将trade_date转换为frame
                        data['frame'] = pd.to_datetime(data['trade_date'])
                        # 如果需要保留原始trade_date列，可以注释下面这行
    ...
    def _convert_to_json(self, data: pd.DataFrame) -> List[str]:
        ...
        """
        # 将日期转换为字符串
        if 'frame' in data.columns and data['frame'].dtype == 'datetime64[ns]':
            data = data.copy()
            data['frame'] = data['frame'].dt.strftime('%Y-%m-%d')
        elif 'trade_date' in data.columns and data['trade_date'].dtype == 'datetime64[ns]':
            data = data.copy()
